...
class db
{
	...
	public static function db($table=FALSE,$orm=FALSE,$connection='default'){
	// If no table given, return connection
	if(!$table) return self::connection($connection);
	
	// Otherwise, return table
	return ($orm) ? self::connection($connection)->table($table)->orm($orm) : self::connection($connection)->table($table);
	}
